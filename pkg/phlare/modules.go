...
func (f *Phlare) initRuntimeConfig() (services.Service, error) {
	...
	f.Server.HTTP.Methods("GET").Path("/api/v1/tenant_limits").Handler(middleware.AuthenticateUser.Wrap(validation.UserLimitsHandler(f.Cfg.LimitsConfig, f.TenantLimits)))
	return serv, err
}
